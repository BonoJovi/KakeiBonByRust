...
**Total Tests**: 726 (jest suites; 23 test files, per `npm test`)
...
### Feature-Specific Tests (303 tests)
...
- **aggregation-error-translate.test.js** - `translateAggregationError` shape-guard tests (Fable-5 #9): coerces `Err(String)` / `ApiError { code, message }` / `Error` to a substring-matchable string before routing to i18n keys, so the aggregation banner never renders `"[object Object]"` (11)
