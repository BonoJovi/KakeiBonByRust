...
### 3. 明細テーブル（TRANSACTION_DETAILS）

```sql
CREATE TABLE TRANSACTION_DETAILS (
    DETAIL_ID INTEGER PRIMARY KEY,
    HEADER_ID INTEGER NOT NULL,
    CATEGORY2_CODE VARCHAR(50),             -- 中分類
    CATEGORY3_CODE VARCHAR(50),             -- 小分類
    AMOUNT INTEGER NOT NULL,                -- 本体価格（税抜き）
    TAX_RATE DECIMAL(5,2) DEFAULT 8.00,     -- 税率（%）デフォルト8%
    TAX_AMOUNT INTEGER,                     -- 税額（自動計算、手動修正可）
    ITEM_DESCRIPTION TEXT NOT NULL,         -- 商品名
    ITEM_MEMO TEXT,
    DISPLAY_ORDER INTEGER,
    ENTRY_DT DATETIME NOT NULL,
    UPDATE_DT DATETIME,
