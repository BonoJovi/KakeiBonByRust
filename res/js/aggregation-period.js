...
console.log('aggregation-period.js loaded');
...
function initializeFilterDefaults() {
    ...
    startDateInput.value = AggCommon.formatDate(firstDayOfMonth);
    endDateInput.value = AggCommon.formatDate(today);
}
...
async function executeAggregation() {
    ...
    if (!startDateStr || !endDateStr) {
        showMessage('error', i18n.t('aggregation.error_no_dates') || 'Please select start and end dates');
        return;
    }
    ...
    if (startDate > endDate) {
        showMessage('error', i18n.t('aggregation.error_invalid_date_range') || 'Start date must be before end date');
        return;
    }
