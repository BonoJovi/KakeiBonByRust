...
[![Version](https://img.shields.io/badge/Version-2.1.0-blue)](https://github.com/BonoJovi/KakeiBonByRust/releases/tag/v2.1.0)
[![Rust](https://img.shields.io/badge/Rust-1.70+-orange.svg)](https://www.rust-lang.org/)
[![Tauri](https://img.shields.io/badge/Tauri-v2.9.3-blue.svg)](https://tauri.app/)
[![Tests](https://img.shields.io/badge/tests-845%20passing-brightgreen.svg)](#test-results--テスト結果)
...
**Ver.2.1.0を正式リリースいたしました！**

Ver.2.1.0 は、毎月の給料・公共料金・サブスクリプションなどの「定期的に発生する予定入出金」を**ルール 1 件登録するだけで期間内の予定取引を一括生成できる**ようにするマイナーリリースです。家計の月次サイクルにフィットする中核機能として、年金や給料日のような土日祝シフト（前倒し／後倒し）にも対応しました。

主な新機能：

- **繰り返し予定入出金ルール**: 周期＋期間＋テンプレート（金額・カテゴリ・口座・メモ）を登録すると、対応期間内の予定取引が一括 INSERT される
- **対応サイクル**（v2.1.0 UI から作成可能）: 日次（n 日毎、起点日指定）／月次（n 月毎、日付指定）／月次（n 月毎、**第N週指定曜日**）。Weekly・Yearly・月末モードはバックエンド完成済みで UI は v2.1.x 以降
- **第N週指定曜日**: 「毎月第4木曜日」のような指定が可能。Zaim 等競合では未対応の差別化要因（KakeiBon を作った主要動機の 1 つ）
- **土日祝シフト**: 生成日が土日祝に当たった場合、「前倒し（給料日想定）」「後倒し（引落想定）」「変更なし」の 3 モードから選択。連休も平日まで自動的に遡る／進む
- **日本祝日マスタの自動生成**: jpholiday クレートを使い、起動時に当年 -5 / +10 年（16 年分）の祝日を自動転記
- **ルール一覧 + 削除（2 モード）**: 登録済みルールの確認、削除時に「ルールのみ削除（生成済み予定取引は単独取引として残す）」「ルールと予定取引を全て削除」を選択可能

`TRANSACTIONS_HEADER` への `RULE_ID` カラム追加と関連テーブル新設を含むスキーマ変更がありますが、既存データへの破壊的変更はなく、起動時のマイグレーションで自動対応します。
...
**【次期バージョン予定】** Ver.2.2.0 では、Ver.2.1.0 の繰り返しロジックを流用した「集計起算日カスタマイズ」（給料日や年金振込日に合わせて月次サイクルを設定）を実装予定です。Ver.2.1.x では Weekly / Yearly UI、ルール編集機能、祝日カスタマイズ UI などの追加を予定しています。
...
**[Next Versions]** Ver.2.2.0 will reuse the recurrence logic from Ver.2.1.0 to add *aggregation cycle start day customization* (align the monthly cycle with payday or pension transfer dates). Ver.2.1.x is planned to add the weekly / yearly UI, rule editing, and the user-defined holiday UI.
...
| Metric | Count |
|--------|-------|
| 👁️ **Total Views** / 総閲覧数 | **135** |
| 📦 **Total Clones** / 総クローン数 | **1,777** |

*Last Updated / 最終更新: 2026-05-08 01:43 UTC*
