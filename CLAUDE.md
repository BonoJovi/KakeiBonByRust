...
4. **Release**: Edit 3 version files manually (Cargo.toml, tauri.conf.json, package.json); `Cargo.lock` is the 4th, rewritten automatically by `cargo build` — stage the diff. Then run `check-release.sh` (use `/release`)
