...
- [Note] ドキュメントの整備
  ...
  - ✅ Phase3: 旧API仕様書削除（完了）
  - [Refresh] Phase4: 設計ドキュメントの整備（進行中）
...
### [Refresh] 進行中: APIリファレンス統合・再構成 (2025-12-05)
...
### [Build]️ 設計ドキュメント整備 (Design Documentation)
...
## [WIP] 費目管理機能 (Category Management)
...
## [Money] 入出金管理機能 (Transaction Management) - TRANSACTION_HEADERS
...
## [Shop] 店舗管理機能 (Shop Management) ✅ 完了
...
## [Bank] 口座管理機能 (Account Management) ✅ 完了
...
#### 実装内容
- [ ] **フロントエンドテスト完全実装**
  - [ ] 月次集計テスト（UI初期化、バリデーション、実行、結果表示）
  - [ ] 日次集計テスト
  - [ ] 週次集計テスト
  - [ ] 年次集計テスト
  - [ ] 期間別集計テスト
  - [ ] 共通モジュール（aggregation-common.js）のテスト
...
#### テストファイル
- `res/tests/aggregation-test-helpers.js` ✅ 完了
- `res/tests/aggregation-monthly.test.js` ⏳ スケルトンのみ
- `res/tests/aggregation-daily.test.js` 未作成
- `res/tests/aggregation-weekly.test.js` 未作成
- `res/tests/aggregation-yearly.test.js` 未作成
- `res/tests/aggregation-period.test.js` 未作成

**見積もり**: 4-6時間

---

### 2. IS_DISABLED機能の実装（全管理画面共通）
...
- [ ] **入出金明細管理機能**（TRANSACTION_DETAILS）
  ...
  - **Phase 9: ドキュメント**
    - [ ] API仕様書（日英）
    - [ ] 明細管理機能仕様書（日英）
## [Chart] 集計・レポート機能 ✅ 完了 (2025-11-21)

### Phase 4: 集計機能実装完了
**完了日**: 2025-11-21 04:51 JST
...
- ✅ **年次集計** (Yearly Aggregation)
  - 年 + 年度開始月で指定
  - 暦年（1月開始）と会計年度（4月開始）に対応
  - 日本の会計年度対応
  
- ✅ **期間別集計** (Period Aggregation)
  - 開始日〜終了日で自由に指定
  - 最も柔軟な集計方法
  - 日付範囲バリデーション
  - デフォルト: 今月1日〜今日
...
- ✅ **結果表示の統一**
  - 共通モジュール（aggregation-common.js）で結果表示
  - グループ名、合計金額（純増減）、件数、平均金額
  - 支出は減算、収入は加算、振替は金額0で除外
  - 口座別集計のみ振替含む（説明ノート表示）
...
#### バックエンドアーキテクチャ
- ✅ **3階層設計**
  - コア関数: `monthly_aggregation()`, `daily_aggregation()`, etc.
  - ラッパー関数: `execute_monthly_aggregation()`, etc.
  - Tauriコマンド: `get_monthly_aggregation`, etc.
  
- ✅ **型安全設計**
  - `DateFilter` Enum (Exact, Between)
  - `GroupBy` Enum (Category1/2/3, Account, Shop)
  - `OrderField`, `SortOrder` Enum
  - `AggregationRequest`, `AggregationResult` 構造体
  
- ✅ **動的SQL生成**
  - フィルタ条件に応じた動的WHERE句生成
  - グルーピング軸に応じた動的SELECT/GROUP BY句生成
  - ソート条件に応じた動的ORDER BY句生成

#### テスト状況
- ✅ テストヘルパー実装 (`aggregation-test-helpers.js`)
- ✅ 月次集計テストスケルトン実装
- ⏳ 詳細テストケース実装（今後）
...
**注記**: 基本的な集計機能は完成。データが蓄積されてからの実用テストで更なる改善を行う。
...
## [Soon] 今後の実装予定
  ...
  **設計方針**:
  - **3階層アーキテクチャ**: フロントエンド（UI） → ラッパー関数（ビジネスロジック） → コア関数（SQLジェネレータ）
  - **ソート機能をコア関数に持たせる理由**:
    1. **責務の単純化**: フロントエンドでのソート実装が不要、各画面での個別実装を回避
    2. **データハンドリングの効率化**: DBから取得時点でソート済み、メモリ上での再ソート不要
    3. **パフォーマンス**: SQL側でソート（インデックス活用）の方が大量データに有利
    4. **一貫性**: 全画面で同じソートロジックを使用、バグの混入を防ぐ
  - **責務分離（SRP）**:
    - **フロントエンド（UI層）**: 表示・ユーザー操作・パラメータ収集のみ
      - ソート条件の選択UIを提供
      - 取得したデータをそのまま表示
      - ページネーション等の表示制御
...
## [Target] 端数処理の自動検出機能の実装詳細 (2025-11-18)
...
最終更新: 2025-11-21 04:51 JST

## [Map]️ 中長期ロードマップ（2026年前半）

詳細は [RELEASE_ROADMAP.md](docs/RELEASE_ROADMAP.md) を参照

### v1.1.0 - ダッシュボード機能（2026年1月7日）✅ リリース済み
- Chart.jsによるグラフ表示
- 月別収支推移グラフ
- カテゴリ別円グラフ

### v1.1.1 - セキュリティ・安定性向上（2026年1月下旬）
- Dependabot警告対応
- CI/CD改善（リリース前インポート完全性チェック）
- パフォーマンス改善（軽微）

### v1.2.0 - CSV入出力機能（2026年1月下旬〜2月上旬）
- CSVエクスポート（全データ/期間指定/エンコーディング選択）
- CSVインポート（バリデーション/エラーレポート/ロールバック）

### v1.3.0 - 財務諸表機能（2026年2月中旬）
- バランスシート（貸借対照表）
- 損益計算書（P&L）
- キャッシュフロー計算書

### v1.4.0 - ARIA対応：データ入力画面（2026年2月下旬〜3月上旬）
- ♿ スクリーンリーダー対応（入出金入力画面）
  - ARIA属性追加
  - キーボードナビゲーション完全対応
  - フォーカス管理改善
...
### v1.4.1 - ARIA対応：マスタデータ画面（2026年3月中旬〜4月上旬）
- 費目管理画面のARIA対応（**最も複雑**）
- 口座管理画面のARIA対応
- 店舗管理画面のARIA対応
- メーカー管理画面のARIA対応
...
### v1.4.2 - ARIA対応：集計・ダッシュボード画面（2026年4月中旬〜4月下旬）
- 各種集計画面のARIA対応
- ダッシュボード画面のARIA対応
- グラフの代替テキスト対応

### v1.5.0 - 多言語対応拡充（2026年5月以降）
- 中国語（簡体字）
- 韓国語
- フランス語（コントリビューター次第）
...
## [Access] ARIA読み上げフィールド設計（v1.4.0〜v1.4.2 全マスタデータ共通）

**最終更新**: 2026-01-22
**背景**: 日本語の漢字は音読み・訓読みの曖昧性があるため、スクリーンリーダーが正しく読み上げられない問題がある

### 対象テーブル（ユーザー追加可能な全マスタデータ）

| テーブル | 対応バージョン | 備考 |
|---------|---------------|------|
| CATEGORY1/2/3（費目） | v1.4.1 | ボリューム最大、最も複雑 |
| ACCOUNTS（口座） | v1.4.1 | |
| SHOPS（店舗） | v1.4.1 | |
| MANUFACTURERS（メーカー） | v1.4.1 | |
| PRODUCTS（商品） | v1.4.1 | |
...
| フィールド | 必須/任意 | 用途 |
|-----------|----------|------|
| **reading（よみがな）** | 必須 | ARIA読み上げ、日本語ソート |
| **name_ja（表示名）** | 任意 | 漢字表記（画面表示用） |
| **name_en（英語名）** | 既存 | 英語表示、英語ARIA |

#### 表示ロジック
```javascript
// 画面表示
displayName = item.name_ja || item.reading;

// ARIA読み上げ（日本語ロケール）
ariaLabel = item.reading;

// ARIA読み上げ（英語ロケール）
ariaLabel = item.name_en;
```

#### バリデーションルール
- **DBレベル**: 複数項目間の入力チェック不可のため、チェックなし
- **アプリレベル**: 以下のロジックを実装
  - `reading`が空の場合: エラー（日本語項目の場合）
  - `name_ja`が空の場合: `reading`をフォールバック表示
  - 英語のみの項目: `reading`不要（スクリーンリーダーが英語を処理可能）

#### ユースケース
1. **アクセシビリティ（メイン）**: 視覚障がい者向けのスクリーンリーダー対応
2. **言語学習（副次的）**:
   - 日本語学習者: 漢字＋よみがな＋英語で学習可能
   - 双方向学習: 日→英、英→日の両方に対応

### 実装時の注意点

#### DBマイグレーション
- 全対象テーブルに`READING`カラム追加（NULL許容で追加後、段階的に必須化）

#### 既存データのマイグレーション
- **デフォルトデータ（未変更）**: 開発側でマイグレーションスクリプトにより`reading`を一括設定
- **ユーザー追加・変更データ**: ユーザーに`reading`を入力してもらう
  - 開発側提供のデフォルトデータも、ユーザーが名称変更している可能性がある
  - 例: 「食費」→「食事代」に変更済みの場合、「しょくひ」では不整合
  - したがって、全ての追加・変更データで`reading`をユーザーに入力してもらう
- **判定ロジック**: `name_ja`がデフォルト値と一致するかで変更有無を判定
- **よみがな未設定検出機能**:
  - アプリ起動時または画面表示時に`reading`が空のデータを検出
  - 通知バッジまたは警告メッセージで入力を促す
  - 例: 「よみがなが未設定の項目が127件あります」
  - 一括入力画面の提供を検討（効率化のため）

#### UI更新
- 全マスタデータ管理画面に`reading`入力フィールド追加
- 新規追加時: `reading`を必須入力に
- 既存データ編集時: `reading`が空なら入力を促す（警告表示）

#### ユーザー設定：名称表示モード
- **設定フィールド**: `NAME_DISPLAY_MODE`（ユーザーテーブルまたは設定テーブル）
- **表示モード**:
  | モード | 表示構成 | 表示例 | 用途 |
  |--------|----------|--------|------|
  | `JA_ONLY` | `name_ja` | 食費 | 通常の日本語表示 |
  | `JA_READING_EN` | `name_ja` + `reading` + `name_en` | 食費（しょくひ）Food | 双方向学習（日↔英） |
  | `READING_EN` | `reading` + `name_en` | しょくひ / Food | 英語→日本語学習者 |
  | `EN_ONLY` | `name_en` | Food | 英語のみ |
- **ARIA読み上げ**:
  | モード | 読み上げ構成 | 読み上げ例 |
  |--------|--------------|------------|
  | `JA_ONLY` | `reading` | しょくひ |
  | `JA_READING_EN` | `reading` + `name_en` | しょくひ、Food |
  | `READING_EN` | `reading` + `name_en` | しょくひ、Food |
  | `EN_ONLY` | `name_en` | Food |
- **設定UI**: 設定画面に表示モード選択を追加

#### 事前準備（v1.2.0/v1.3.0と並行作業可）
- [ ] **1. マイグレーションスクリプト作成**: READINGカラム追加SQL
- [ ] **2. 変更判定用のデフォルト値リスト作成**: name_jaの初期値一覧
- [ ] **3. デフォルトデータのreading洗い出し**:
  - [ ] 費目（CATEGORY1/2/3）: 約120件
  - [ ] 口座テンプレート
  - [ ] その他マスタ
- [ ] **4. readingデータファイル作成**: CSVまたはSQLで管理

#### その他
- **i18nシステム**: 既存のi18nシステムとの整合性を保つ
- **テスト**: 各画面でのARIA読み上げ動作確認が必要

---

最終更新: 2025-12-06 08:50 JST

## [Fix] v1.1.1 追加タスク（リリース品質向上）

### CI/CD改善：リリース前インポート完全性チェック

**目的**: libディレクトリ無しでもビルド・テストが成功することを保証

**実装内容**:
1. ワークフローに `test-release-build` ジョブ追加
   - libディレクトリを一時削除
   - `cargo build --release`
   - `cargo test --release`
   - 全て成功したらlibを復元
