...
### 🚧 進行中
- [ ] メーカー管理機能（MANUFACTURERS）
  - [x] バックエンド実装完了（CRUD操作、名前重複チェック）
  - [x] フロントエンド実装完了（Modal統合、翻訳リソース）
  - [ ] UI手動テスト未完了
  - [ ] IS_DISABLED機能未実装（次のタスク）
- [ ] 商品管理機能（PRODUCTS）
  - [x] バックエンド実装完了（CRUD操作、名前重複チェック、メーカーとの関連）
  - [x] フロントエンド実装完了（Modal統合、翻訳リソース）
  - [ ] UI手動テスト中（IS_DISABLED問題を発見）
  - [ ] IS_DISABLED機能未実装（次のタスク）

**注記**:
- 2025-11-11: メーカー・商品管理のバックエンド/フロントエンド実装完了
- 手動テスト中にIS_DISABLED=1のデータが一覧に表示されない問題を発見
- 対応：モーダルにチェックボックスを追加し、IS_DISABLEDの切り替えを可能にする
- この機能は他の管理画面（店舗、口座、費目など）でも必要
